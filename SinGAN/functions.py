...
def gen_fake(real, fake_background, mask, constraint, mask_source, opt, border = False, mask_loc = None):
    ...
    for i in range(opt.batch_size):
        ...
        constraint_ind = torch.zeros((1, 3, im_height, im_width)) 
        
        #coloring eye
        # eye_colored = eye.clone()
        # print(eye_colored.size())
        # plt.imshow(eye_colored.cpu().detach().squeeze().permute(1,2,0), cmap="gray")
        # plt.show()
...
def make_input(noise, mask, constraint, opt):
    ...
    constraint = pad_mask(constraint[:, :, :, :], (noise_height, noise_width)).float().repeat(opt.batch_size, 1, 1, 1).clone()
    ...
    G_input = torch.cat((noise, mask, constraint), dim=1) # concatenating to make input to generator
    return G_input                 
    ...
    if len(Gs) > 0:
        if mode == 'rand':
            ...
            for G,Z_opt,real_curr,real_next, mask_curr, constraint_curr, noise_amp in zip(Gs,Zs,reals,reals[1:], masks, constraints, NoiseAmp):
                ...
                G_input = make_input(z_in, mask_curr, constraint_curr, opt)
